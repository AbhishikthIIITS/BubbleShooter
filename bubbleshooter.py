...
bgcolor = vblue
# clrlist = [black,grey,blue,red,pink,lightpink,hotpink,
#            deeppink,grapecolor,amber,comic,lytgray,
#           peach,green,GRAY,white]
clrlist = [red, green]
...
class Ary(pygame.sprite.Sprite):
    ...
    def update(self, dir):
        ...
        # Move arrow based on angle and speed
        radians = math.radians(self.angle)
        dx = math.cos(radians) * self.speed
        dy = -math.sin(radians) * self.speed

        # Check for collisions with walls and adjust movement
        if self.rect.centerx + dx < 0 or self.rect.centerx + dx > winwdth:
            dx = 0
        if self.rect.centery + dy < 0 or self.rect.centery + dy > winhgt:
            dy = 0

        self.rect.move_ip(dx, dy)

        # Calculate endpoint of pointing line
        end_x = self.rect.centerx + int(math.cos(math.radians(self.angle)) * self.arrow_length)
        end_y = self.rect.centery - int(math.sin(math.radians(self.angle)) * self.arrow_length)

        # Draw pointing line
        pygame.draw.line(dispsurf, black, (self.rect.centerx, self.rect.centery), (end_x, end_y), 2)
...
def rngame():
    ...
    musclist = ['Indias Biggest Criminal BGM.mp3']
